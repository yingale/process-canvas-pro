 ...
 * Used inside StepPropertiesPanel when a step has a moduleRef.
 */
...
export default function ModuleConfigPanel({ moduleRef, basePath, onPatch }: ModuleConfigPanelProps) {
  ...
  const handleSave = () => {
    onPatch([
      {
        op: "replace",
        path: `${basePath}/moduleRef/instanceConfig`,
        value: config,
      },
    ]);
    setDirty(false);
  ...
  return (
    <div className="module-config-panel">
      ...
      {schema.length === 0 ? (
        <div className="px-4 py-6 text-center text-[11px] text-foreground-subtle">
